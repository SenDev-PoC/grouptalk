...
async function main() {
  ...
  const teacher = createClient(apiUrl, anonKey, { auth: { persistSession: false } })
  const auth = await teacher.auth.signUp({
    email: `server-owned-${randomUUID()}@example.com`,
    password: `Secure-${randomUUID()}!`,
  })
  assert.ifError(auth.error)
  assert.ok(auth.data.user && auth.data.session)
  const teacherId = auth.data.user.id
  await teacher.realtime.setAuth(auth.data.session.access_token)
  ...
  try {
    ...
    const teacherInsight = await teacher
      .from('group_insights')
      .select('group_id')
      .eq('group_id', groupId)
      .single()
    assert.ifError(teacherInsight.error)
    assert.equal(teacherInsight.data.group_id, groupId)

    const forbiddenInsightWrite = await teacher
      .from('group_insights')
      .update({ summary: 'forged' })
      .eq('group_id', groupId)
    assert.ok(forbiddenInsightWrite.error, 'browser group_insights UPDATE must fail')

    const forbiddenUtteranceWrite = await teacher.from('utterances').insert({
      session_id: sessionId,
      group_id: groupId,
      text: 'forged',
    })
    assert.ok(forbiddenUtteranceWrite.error, 'browser utterances INSERT must fail')
    ...
    const updateInsight = () =>
      executeServerSql(
        databaseUrl,
        `update public.group_insights set updated_at = clock_timestamp() where group_id = '${groupId}'`,
      )
    updateInsight()
    const retryUpdate = setInterval(updateInsight, 1_000)
    try {
      await Promise.race([
        received,
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Realtime UPDATE event was not received')), 15_000),
        ),
      ])
    } finally {
    ...
    console.log('  ok  소유 교사 SELECT 유지 및 server-owned table 쓰기 차단')
    console.log('  ok  PostgreSQL 서버 연결 쓰기 유지')
    console.log('  ok  group_insights Realtime UPDATE 수신 유지')
  } finally {
    if (channel) await teacher.removeChannel(channel)
    await teacher.realtime.disconnect()
    executeServerSql(
      databaseUrl,
      `delete from public.activities where id = '${activityId}'`,
    )
    executeServerSql(databaseUrl, `delete from auth.users where id = '${teacherId}'`)
  }
}
